            ...
            ->schema([
                Section::make('Detalhes do Site')
                    ->schema([
                        ...
                        TextInput::make('footer_number')
                            ->label('Numero do footer'),
                        TextInput::make('footer_email')
                            ->label('Email do footer'),
                    ])->columns(2),
                ...
                Section::make('Logos')
                    ->schema([
                        ...
                        FileUpload::make('software_logo_white')
                            ...
                            ->image(),
                        // FileUpload::make('software_logo_loading')
                        //     ->label('Logo loading')
                        //     ->placeholder('Carregue uma logo para o loading')
                        //     ->image(),
                        FileUpload::make('software_logo_black')
                            ->label('Logo Escura')
                            ->placeholder('Carregue uma logo escura')
                            ->image(),
                        FileUpload::make('logo_loading')
                            ->label('Logo da tela de loading')
                            ->placeholder('Carregue uma para tela de loading')
                            ->image(),
                    ])->columns(3),
    ...
    public function submit(): void
    {
        try {
            ...
            if(!empty($setting)) {
                ...
                $logoLoading = $this->data['logo_loading'];
                ...
                if (is_array($logoLoading) || is_object($logoLoading)) {
                    if(!empty($logoLoading)) {
                        $this->data['logo_loading'] = $this->uploadFile($logoLoading, 'loading');
                    }
                }
                ...
                $envs->setKeys([
                    'FOOTER_TEXT' => $this->data['footer_text'],
                ]);

                $envs->setKeys([
                    'FOOTER_NUMBER' => $this->data['footer_number'],
                ]);

                $envs->setKeys([
                    'FOOTER_EMAIL' => $this->data['footer_email'],
                ]);
